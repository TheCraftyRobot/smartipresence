...
function connect(){
   ...
   if(search_params.has('accessKey')) {
      ...
      if(validateAccessKey(providedKey)){
         console.log("url access key validated");
         accessKey = providedKey;
      }else{
         console.log("DONT FORM CONNECTION< ACCESS KEY BAD");
  ...
  console.log("next xhr request");
   ...
   var data = JSON.stringify({"botName": accessKey, "owner": "Ross", "botKey": accessKey });
   xhr.send(data);
