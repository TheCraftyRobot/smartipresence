...
function colorOut(color){
	forwardBtn.style.background=color;
 	backwardBtn.style.background=color;
 	rightBtn.style.background=color;
 	leftBtn.style.background=color;
 	stopBtn.style.background=color;
}
...
  //  conText.value += "Message received from to the server : " + message.data + "\n";
    ...
    switch(message.data){
      ...
      case "1":
    	  ...
    	  break;
      case "2":
    	  echoText.value += "Please wait for video to load before using controls";
    	  break;
      case "3":
    	  //ka msg
    	//  echoText.value += "ka";
